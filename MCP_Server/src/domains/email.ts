      ...
      if (textMatch && textMatch[1]) {
        ...
        if (source.includes('quoted-printable')) {
          textContent = textContent
            ...
            .replace(/=([0-9A-F]{2})/g, (_match: string, hex: string) => String.fromCharCode(parseInt(hex, 16))); // Decode hex
        }
...
export async function handleEmailTool(name: string, args: any): Promise<any> {
  switch (name) {
    case "read_emails": {
