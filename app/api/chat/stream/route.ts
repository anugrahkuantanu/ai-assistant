...
export async function POST(req: Request) {
  try {
    ...
    const { messages, newMessage, chatId, modelConfig } =
      (await req.json()) as ChatRequestBody & { modelConfig?: any };
            ...
            if (event.event === "on_chat_model_stream") {
              ...
              if (token) {
                ...
                if (typeof text === 'string') {
                  ...
                  if (typeof firstText === 'object' && 'text' in firstText) {
                    await sendSSEMessage(writer, {
                      type: StreamMessageType.Token,
                      token: firstText.text,
                    });
                  }
                }
              }
